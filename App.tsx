...
function App() {
  ...
  // --- INITIALIZATION: Fetch from Cloud (Supabase) ---
  useEffect(() => {
    const initApp = async () => {
      setInitialLoading(true);
      try {
        // 1. Fetch Dynamic Prefixes First
        const prefixes = await fetchBillPrefixes();
        if (prefixes) {
            updateGlobalPrefixMap(prefixes);
        }
        ...
        if (cloudResult && cloudResult.data && cloudResult.data.length > 0) {
           ...
           // 3. Background Sync Contacts (To ensure latest phones)
           setSyncStatus('syncing');
           loadedData = await syncPartyMobileNumbers(loadedData, (status) => {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      if (headerDownloadMenuRef.current && !headerDownloadMenuRef.current.contains(event.target as Node)) {
        setIsHeaderDownloadMenuOpen(false);
      }
  ...
  const handleFileSelect = async (file: File) => {
    ...
    try {
      ...
      const syncedProcessed = await syncPartyMobileNumbers(processed, (status, msg) => {
        setSyncStatus(status);
        if (status === 'error') {
            addToast('error', `Contact Sync failed: ${msg}`);
        }
      });
      ...
      if (e.target.files && e.target.files[0]) {
          ...
          try {
              ...
              if (uploadRes.success) {
                  ...
                  addToast('error', 'Failed to save Prefixes to Cloud');
              }
          } catch (err) {
              console.error(err);
              addToast('error', 'Invalid Prefix File Format');
          } finally {
              setLoading(false);
              setIsPrefixMenuOpen(false);
              e.target.value = '';
  ...
  const handleClear = async () => {
    ...
    setSyncStatus('idle');
    setLoading(false);
    addToast('info', 'Cloud Data Cleared');
  };
  ...
  const handlePhoneNumberChange = (id: string, value: string, partyName: string) => {
    ...
    setData(updatedData);
    setPhoneSaveStatus(prev => ({ ...prev, [id]: 'saving' }));
    ...
    saveTimeouts.current[id] = setTimeout(async () => {
        ...
        if (res.success) {
            ...
            setPhoneSaveStatus(prev => ({ ...prev, [id]: 'error' }));
            addToast('error', 'Failed to save phone number');
        }
    }, 1000);
  };
  ...
  const openWhatsApp = (phone: string, message: string) => {
    ...
    if (!cleanPhone) {
      addToast('error', "Invalid or missing phone number");
      return;
    }
  ...
  const handleGenerateReminder = async (party: ProcessedParty, directSend: boolean = false) => {
      ...
      try {
        ...
        addToast('error', "Failed to generate reminder.");
      } finally {
  ...
  const handleBulkWhatsAppClick = () => {
    ...
    Array.from(selectedPartyIds).forEach(async (partyId) => {
        const id = String(partyId); // Explicitly ensure string for computed property
  ...
  const DownloadDropdown = ({ isOpen, setIsOpen, menuRef, buttonClass }: { isOpen: boolean, setIsOpen: (v: boolean) => void, menuRef: React.RefObject<HTMLDivElement>, buttonClass?: string }) => (
      <div className="relative" ref={menuRef}>
          <button 
              ...
              className={buttonClass || "flex items-center gap-2 px-3 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-colors"}
          >
              <Download size={16} /> Download <ChevronDown size={14} className={`transition-transform ${isOpen ? 'rotate-180' : ''}`} />
          </button>
          {isOpen && (
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-indigo-600 p-2 rounded-lg text-white shadow-sm"><Globe size={20} /></div>
            <div className="flex flex-col">
                <h1 className="text-lg font-bold text-slate-800 leading-tight">Yash Marketing</h1>
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1">
                        {cloudStatus === 'connected' && <Cloud size={10} className="text-emerald-500" />}
                        {cloudStatus === 'saving' && <RefreshCw size={10} className="text-indigo-500 animate-spin" />}
                        {cloudStatus === 'error' && <CloudOff size={10} className="text-red-500" />}
                        <span className={`text-[10px] font-bold uppercase tracking-wide ${cloudStatus === 'connected' ? 'text-emerald-600' : cloudStatus === 'error' ? 'text-red-500' : 'text-indigo-500'}`}>
                             {cloudStatus === 'saving' ? 'Saving...' : cloudStatus === 'error' ? 'Offline' : 'Cloud Active'}
                        </span>
                    </div>
                    {lastUploadTime && (
                        <>
                            <span className="text-slate-300">|</span>
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Data: {lastUploadTime}</span>
                        </>
                    )}
                </div>
            </div>
             ...
             {selectedPartyId ? (
                ...
                <>
                   <div className="relative" ref={prefixMenuRef}>
                        <button onClick={() => setIsPrefixMenuOpen(!isPrefixMenuOpen)} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors border border-slate-200">
                            <Download size={16} /> Prefix Guide <ChevronDown size={14} />
                        </button>
                        {isPrefixMenuOpen && (
                            <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in">
                                ...
                                <label className="w-full text-left px-4 py-2.5 text-xs font-medium text-indigo-700 hover:bg-indigo-50 transition-colors flex items-center gap-2 cursor-pointer">
                                    <Upload size={14} /> Update Guide
                                    <input ref={prefixInputRef} type="file" accept=".xlsx,.xls" className="hidden" onChange={handlePrefixUpload} />
                                </label>
                            </div>
                        )}
                   </div>
                   {data.length > 0 && (
                       <>
                          <DownloadDropdown isOpen={isHeaderDownloadMenuOpen} setIsOpen={setIsHeaderDownloadMenuOpen} menuRef={headerDownloadMenuRef} buttonClass="flex items-center gap-2 px-3 py-2 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg shadow-sm transition-colors" />
                          ...
                          <button onClick={handleClear} className={`flex items-center gap-2 px-3 py-2 text-xs font-medium rounded-lg transition-colors border ml-2 cursor-pointer ${confirmClear ? 'bg-red-600 text-white border-red-700 hover:bg-red-700 shadow-md' : 'text-red-600 bg-red-50 hover:bg-red-100 border-red-100'}`}>
                             <Trash2 size={16} /> {confirmClear ? 'Confirm Delete?' : 'Delete'}
                          </button>
                       </>
                   )}
                </>
        ...
        {data.length === 0 ? (
           <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in">
              <div className="text-center mb-8">
                  <h2 className="text-3xl font-bold text-slate-900 mb-2">Cloud Payment Console</h2>
                  <p className="text-slate-500 max-w-md mx-auto">Upload your Excel/CSV here. It will be synced to the cloud and accessible from any device.</p>
              </div>
              <FileUpload key={uploadKey} onFileSelect={handleFileSelect} />
              {loading && <p className="mt-4 text-indigo-600 animate-pulse font-medium text-sm">Processing & Syncing to Cloud...</p>}
           </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-20">
                    <div className="p-4 border-b border-slate-200 flex flex-col xl:flex-row xl:items-center justify-between gap-4 bg-white sticky left-0">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <DownloadDropdown isOpen={isDownloadMenuOpen} setIsOpen={setIsDownloadMenuOpen} menuRef={downloadMenuRef} />
                                    ...
                                    <div className="absolute left-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden animate-fade-in">
                                        ...
                                        <div className="max-h-72 overflow-y-auto py-1">
                                            <label className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 cursor-pointer transition-colors group border-b border-slate-100 sticky top-0 bg-white z-10 shadow-sm">
                                                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked={companiesList.length > 0 && filterCompanies.length === companiesList.length} onChange={handleSelectAllCompanies} />
                            ...
                            <div className="relative" ref={dateFilterRef}>
                                <button onClick={() => setIsDateDropdownOpen(!isDateDropdownOpen)} className={`flex items-center gap-2 px-3 py-2 bg-slate-700 text-white text-sm rounded-lg hover:bg-slate-800 transition-colors font-medium border-none outline-none ${(dateRange.from || dateRange.to) ? 'ring-2 ring-indigo-500' : ''}`}>
                                    <Calendar size={16} />
                                    <span>{(dateRange.from && !dateRange.to) ? `Up to ${formatDisplayDate(dateRange.from)}` : (dateRange.from || dateRange.to) ? `${formatDisplayDate(dateRange.from) || '..'} - ${formatDisplayDate(dateRange.to) || '..'}` : 'Date Range'}</span>
                                    <ChevronDown size={14} className={`transition-transform ${isDateDropdownOpen ? 'rotate-180' : ''}`} />
                                </button>
                                {isDateDropdownOpen && <DateRangePicker startDate={dateRange.from} endDate={dateRange.to} onChange={(range) => setDateRange(range)} onClose={() => setIsDateDropdownOpen(false)} />}
                            </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-slate-200">
                            <thead className="bg-slate-50">
                                <tr>
                                    <th className="px-6 py-3 w-10"><input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked={filteredData.length > 0 && selectedPartyIds.size === filteredData.length} onChange={toggleSelectAll} /></th>
                                ...
                                {filteredData.map((party) => {
                                    ...
                                    return (
                                        <tr key={party.id} className={`hover:bg-slate-50 transition-colors ${selectedPartyIds.has(party.id) ? 'bg-indigo-50/50' : ''}`}>
                                            <td className="px-6 py-4 w-10"><input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked={selectedPartyIds.has(party.id)} onChange={() => toggleSelectOne(party.id)} /></td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <div className="flex flex-col items-end gap-1">
                                                    {visibleDebit > 0 && <span className="text-sm font-bold text-red-600 font-mono block">₹{visibleDebit.toLocaleString()} <span className="text-[9px] opacity-70">DR</span></span>}
                                                    {party.balanceCredit > 0 && <span className="text-sm font-bold text-emerald-600 font-mono block">₹{party.balanceCredit.toLocaleString()} <span className="text-[9px] opacity-70">CR</span></span>}
                                                    {visibleDebit === 0 && party.balanceCredit === 0 && <span className="text-slate-400 font-mono">-</span>}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-2">
                                                  <input type="text" placeholder="10 Digit No" className="bg-slate-100 text-slate-700 placeholder:text-slate-400 border border-slate-200 rounded-md px-3 py-1.5 text-xs font-bold focus:ring-2 focus:ring-indigo-500 outline-none w-32 transition-all" value={party.phoneNumber} onChange={(e) => handlePhoneNumberChange(party.id, e.target.value, party.partyName)} />
                                                  <div className="w-5 h-5 flex items-center justify-center">
                                                      {phoneSaveStatus[party.id] === 'saving' && <Loader2 size={16} className="animate-spin text-indigo-500" />}
                                                      {phoneSaveStatus[party.id] === 'saved' && <CheckCircle size={16} className="text-emerald-500 animate-scale-up" />}
                                                      {phoneSaveStatus[party.id] === 'error' && <span title="Save Failed"><AlertCircle size={16} className="text-red-500" /></span>}
                                                  </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-center">
                                                <div className="flex items-center justify-center gap-2">
                                                  <button onClick={() => handleGenerateReminder(party)} className="p-2 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Preview Message"><MessageSquare size={18} /></button>
                                                  <button onClick={() => handleGenerateReminder(party, true)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors" title="Send Direct WhatsApp"><Send size={18} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
      ...
      {isCompanyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-slate-200 flex justify-between items-center"><h3 className="font-bold text-slate-800">Company Wide Export</h3><button onClick={() => setIsCompanyModalOpen(false)} className="text-slate-500"><X size={20} /></button></div>
                <div className="p-4 overflow-y-auto">
                    <div className="grid grid-cols-1 gap-1">
                        {companiesList.map(c => (
                            <button key={c} onClick={() => handleToggleCompanyModal(c)} className={`flex items-center justify-between p-3 rounded-lg border text-sm font-medium ${selectedCompaniesForModal.includes(c) ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-700'}`}>{c} {selectedCompaniesForModal.includes(c) && <Check size={16} />}</button>
                        ))}
                    </div>
                </div>
      ...
      {activeReminder && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
             <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-scale-up">
                 <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50"><h3 className="font-black text-slate-900 uppercase tracking-tight">AI Draft Message</h3><button onClick={() => setActiveReminder(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button></div>
                 <div className="p-6">
                     ...
                     <div className="mt-6 flex flex-col sm:flex-row gap-3">
                        <button onClick={() => { navigator.clipboard.writeText(activeReminder.message); alert("Copied!"); }} className="flex-1 px-4 py-3 bg-white text-slate-700 border border-slate-200 rounded-xl font-bold text-sm hover:bg-slate-50 transition-colors flex items-center justify-center gap-2"><Copy size={18} /> Copy Text</button>
                        <button onClick={() => openWhatsApp(activeReminder.party.phoneNumber, activeReminder.message)} className="flex-1 px-4 py-3 bg-[#25D366] text-white rounded-xl font-bold text-sm hover:bg-[#128C7E] shadow-lg shadow-emerald-100 transition-all flex items-center justify-center gap-2"><Share2 size={18} /> Send via WhatsApp</button>
                     </div>
                 </div>
             </div>
          </div>
      ...
      {isBulkModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[85vh] animate-scale-up">
                ...
                <div className="p-4 bg-blue-50 border-b border-blue-100 flex gap-3 text-xs text-blue-700"><div className="mt-0.5"><Loader2 size={16} className="animate-spin" /></div><p>AI messages are generating automatically. Click "Send" next to each party to open WhatsApp Web in a new tab.</p></div>
                    ...
                    {Array.from(selectedPartyIds).map(id => {
                        ...
                        return (
                            <div key={id} className="flex items-center justify-between p-4 mb-2 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-indigo-200 transition-colors">
                                <div><h4 className="font-bold text-slate-800 text-sm">{party.partyName}</h4><p className="text-xs text-slate-500 font-mono mt-1">{party.phoneNumber || 'No Phone'}</p></div>
                                <div>
                                    {status.status === 'loading' && <span className="flex items-center gap-2 text-xs font-bold text-indigo-50 text-indigo-500 px-3 py-1.5 rounded-lg"><Loader2 size={14} className="animate-spin" /> Generating...</span>}
                                    {status.status === 'error' && <span className="text-xs font-bold text-red-500 bg-red-50 px-3 py-1.5 rounded-lg">Failed</span>}
                                    {status.status === 'ready' && <button onClick={() => openWhatsApp(party.phoneNumber, status.text || '')} className="flex items-center gap-2 px-4 py-2 bg-[#25D366] text-white text-xs font-bold rounded-lg hover:bg-[#128C7E] shadow-md shadow-emerald-100 transition-all"><Share2 size={14} /> Send WhatsApp</button>}
                                </div>
                            </div>
                        );
                    })}
